...
clc
tic;
jmax = 201; % Set number of discrete points here!
tend = 5e5; % Set ENDTIME HERE! 5e5 virker som en god tid
xstart = 0; % Left boundary. a.
xend = 1; % Right boundary. b.
dx = (xend-xstart)/(jmax-1); % Stepsize
dt = 1e-1;
r = dt/dx^2;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Constants
Da = 4e-7; % Diffusivity provided in task
Db = 2/3*Da;
Dc = 8/15*Da;
...
c0 = 3e-2; % Nucleation constant given in task
...
xspace = linspace(xstart,xend,jmax);
...
figureName = sprintf('2a_jmax%d_dt%0.2f_t%1.0e_c0__%0.2f_Da%1.2e_Db%1.2e_Dc%1.2e.fig', ...
    jmax, dt, t*dt, c0, Da, Db, Dc);

savefig(h,figureName);


savefile = sprintf('sVector_2a_jmax%d_dt%0.2f_t%1.0e_c0__%0.2f_Da%1.2e_Db%1.2e_Dc%1.2e.mat', ...
    jmax, dt, t*dt, c0, Da, Db, Dc);
save(savefile,'s')
toc
